...
namespace COMIGHT
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public void ConvertOfficeFiles()
        {
            ...
            try
            {
                ...
                foreach (string filePath in filePaths) //遍历所有文件
                {
                    ...
                    {
                        // 获取目标Excel文件路径
                        string targetFilePath = Path.Combine(folderPath, $"{Path.GetFileNameWithoutExtension(filePath)}.xlsx"); //获取目标文件路径
                        //获取目标文件路径：如果目标文件不存在，则得到原目标文件路径；否则，在原目标文件主名后添加4位随机数，得到新目标文件路径
                    ...
                    {
                        string targetFilePath = Path.Combine(folderPath, $"{Path.GetFileNameWithoutExtension(filePath)}.docx"); //获取目标Word文件路径
                        //获取目标文件路径：如果目标文件不存在，则得到原目标文件路径；否则，在原目标文件主名后添加4位随机数，得到新目标文件路径
                        targetFilePath = !File.Exists(targetFilePath) ? targetFilePath :
        ...
        public void DisassembleExcelWorkbook()
        {
            try
            {
                // 创建功能选项列表
                ...
                List<string>? filePaths = SelectFiles(EnumFileType.Excel, false, "Select the Excel File"); //获取所选文件列表
                if (filePaths == null) //如果文件列表为null，则结束本过程
                {
                    return;
                }
                ...
                using (ExcelPackage excelPackage = new ExcelPackage(new FileInfo(filePaths[0]))) // 打开Excel工作簿，赋值给Excel包变量
                {
                    ...
                    for (int i = excelWorksheetStartIndex; i <= excelWorksheetEndIndex; i++) //遍历所有指定范围的Excel工作表
                    {
                        ...
                        if (createDataDict) // 如果需要创建数据字典（按列拆分时）
                        {
                            ...
                            {
                                string? keyCellText = excelWorksheet.Cells[columnLetter + j.ToString()].Text; // 获取键值：当前行拆分基准列的单元格值
                                string key = !string.IsNullOrWhiteSpace(keyCellText) ?
                                    keyCellText : "-Blank-"; //获取键值：如果当前键值不为空，则得到得键值本身；否则得到"-Blank-"
                        ...
                        switch (function) //根据功能序号进入相应的分支
                        {
                            case EnumDissambleFunction.DissembleByAColumn: //按列拆分为Excel工作簿
                                ...
                                foreach (KeyValuePair<string, List<ExcelRangeBase>> pair in dataDict) // 遍历字典中的每一个键值对
                                {
                                    using (ExcelPackage targetExcelPackage = new ExcelPackage()) //新建Excel包，赋值给目标Excel包变量（为当前工作表的每个键值对创建一个新工作簿）
                                    {
                                        ...
                                        // 保存目标Excel工作簿文件
                                        FileInfo targetExcelFile = new FileInfo(Path.Combine(targetSubfolderPath, $"{CleanPath($"{pair.Key}_{excelWorksheet.Name}_{excelWorkbookFileMainName}")}.xlsx")); //获取目标Excel工作簿文件路径信息
                            ...
                            case EnumDissambleFunction.DissembleByWorksheets: // 拆分工作表到独立工作簿
                                ...
                                using (ExcelPackage targetExcelPackage = new ExcelPackage()) // 新建Excel包，赋值给目标Excel包变量（为当前工作表创建一个新工作簿）
                                {
                                    ...
                                    // 保存目标Excel工作簿文件
                                    FileInfo targetExcelFile = new FileInfo(Path.Combine(targetSubfolderPath, $"{CleanPath($"{excelWorksheet.Name}_{excelWorkbookFileMainName}")}.xlsx")); //获取目标Excel工作簿文件路径信息
                                    targetExcelPackage.SaveAs(targetExcelFile);
        ...
        private void ExtractTablesFromWordDocuments()
        {
            ...
            try
            {
                ...
                foreach (string filePath in filePaths) // 遍历所有文件
                {
                    ...
                    string targetExcelFilePath = Path.Combine(targetFolderPath, $"{CleanPath($"Tbl_{Path.GetFileNameWithoutExtension(filePath)}")}.xlsx"); // 获取目标Excel文件路径
        ...
        private void ProcessExcelWorksheets()
        {
            ...
            try
            {
                ...
                {
                    currentFilePath = excelFilePath; //将当前Excel文件路径赋值给当前文件路径变量
                ...
                if (targetExcelWorkbookPrefix != null)  //如果目标Excel工作簿前缀不为null（执行功能1-3时，将生成新工作簿并保存）
                {
                    ...
                    FileInfo targetExcelFilePath = new FileInfo(Path.Combine(targetFolderPath!, $"{CleanPath($"{targetExcelWorkbookPrefix}_{targetFileMainName}")}.xlsx")); //获取目标Excel工作簿文件路径信息
        ...
        public void CreatePlaceCards()
        {
            try
            {
                ...
                using (ExcelPackage targetExcelPackage = new ExcelPackage()) //新建Excel包，赋值给目标Excel包变量（目标Excel工作簿）
                {
                    ...
                    // 保存目标工作簿
                    string targetFolderPath = appSettings.SavingFolderPath; // 获取目标文件夹路径
                    string targetFilePath = Path.Combine(targetFolderPath, $"{CleanPath($"PlCd_{Path.GetFileNameWithoutExtension(filePaths[0])}")}.xlsx"); //获取目标Excel工作簿文件路径
        ...
        private async Task CreateFolders()
        {
            try
            {
                ...
                // 创建目标文件夹路径
                string targetFolderPath = Path.Combine(appSettings.SavingFolderPath, CleanPath($"Dir_{Path.GetFileNameWithoutExtension(filePaths[0])}")); //获取目标文件夹路径
                ...
                for (int i = 0; i < dataTable.Rows.Count; i++) //遍历DataTable所有数据行
                {
                    ...
                    for (int j = 0; j < dataTable.Columns.Count; j++) //遍历DataTable所有数据列
                    {
                        ...
                        {
                            newPath = Path.Combine(newPath, CleanPath(Convert.ToString(dataTable.Rows[i][j])!)); //将现有新文件夹路径和当前数据行当前数据列的文件夹名合并，重新赋值给自身
                            CreateFolder(newPath);
                        }
                    }
                }

                ShowSuccessMessage($"Folders created in '{targetFolderPath}'.");
            }
        ...
        private void CreateFileList()
        {
            try
            {
                ...
                while (stack.Count > 0) // 当栈不为空时，继续循环
                {
                    ...
                    foreach (FileInfo file in files) // 遍历每个文件信息
                    {
                            ...
                            (attributes & FileAttributes.Temporary) != FileAttributes.Temporary)
                        {
                            ...
                            dataRow["FilePath"] = file.FullName; // 将文件路径赋值给数据行的FilePath列
                            dataRow["FolderPath"] = file.DirectoryName; // 将文件所在文件夹的路径赋值给数据行的FolderPath列
                            //在文件路径中删除第一级文件夹路径和最末级文件名，并将结果赋值给数据行的Subpath列
                            ...
                            dataRow["LiteralDate"] = (object?)GetEarliestDate(Path.GetFileNameWithoutExtension(file.Name))?.Date ?? DBNull.Value; // 将文件推测生效日期赋值给数据行的Valid列
                ...
                using (ExcelPackage targetExcelPackage = new ExcelPackage()) //新建Excel包，赋值给目标Excel包变量
                {
                    ...
                    string targetFolderPath = appSettings.SavingFolderPath; // 获取目标文件夹路径
                    FileInfo targetExcelFilePath = new FileInfo(Path.Combine(targetFolderPath, $"{CleanPath($"Lst_{folderPath}")}.xlsx")); //获取目标Excel工作簿文件路径信息
        ...
        private void MergeDataIntoDocument()
        {
            ...
            try
            {
                List<string>? filePaths = SelectFiles(EnumFileType.DocumentAndTable, true, "Select Document and Table Files"); //获取所选文件路径列表
                ...
                string targetFolderPath = appSettings.SavingFolderPath; //获取目标文件夹路径
                string targetTxtFilePath = Path.Combine(targetFolderPath, $"{CleanPath($"Mrg_{targetFileMainName}")}.txt"); // 获取目标文本文件的路径全名
