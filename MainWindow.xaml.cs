...
namespace COMIGHT
{
    ...
    public partial class MainWindow : Window
    {
        ...
        //                for (int i = excelWorksheetStartIndex; i <= excelWorksheetEndIndex; i++) //遍历所有指定范围的Excel工作表
        //                {
        //                    if (i > excelPackage.Workbook.Worksheets.Count - 1) // 如果当前工作表索引号大于Excel工作簿内工作表数量-1，则结束本过程（结束遍历当前工作簿的工作表，跳至下一个工作簿）
        //                    {
        //                        break;
        //                    }
        ...
        //                            foreach (KeyValuePair<string, List<ExcelRangeBase>> pair in dataDict) // 遍历字典中的每一个键值对
        //                            {
        //                                using (ExcelPackage assembledExcelPackage = new ExcelPackage()) //新建Excel包，赋值给目标Excel包变量（为当前工作表的每个键值对创建一个新工作簿）
        //                                {
        //                                    ExcelWorksheet collectedExcelWorksheet = assembledExcelPackage.Workbook.Worksheets.Add("Sheet1"); // 新建Excel工作表，赋值给目标工作表变量

        //                                    // 将表头复制到目标Excel工作表
        //                                    if (headerRowCount >= 1) //如果表头行数大于等于1，复制表头
        //                                    {
        //                                        excelWorksheet.Cells[1, 1, headerRowCount, excelWorksheet.Dimension.End.Column].CopyStyles(collectedExcelWorksheet.Cells["A1"]);
        //                                        excelWorksheet.Cells[1, 1, headerRowCount, excelWorksheet.Dimension.End.Column].Copy(collectedExcelWorksheet.Cells["A1"]);
        //                                    }
        ...
        //                                    {
        //                                        //获取目标Excel工作表最末行索引号（如果工作表为空， 则为0）
        //                                        int lastRowIndex = collectedExcelWorksheet.Dimension?.End.Row ?? 0;
        //                                        dictRow.CopyStyles(collectedExcelWorksheet.Cells[lastRowIndex + 1, 1]); //将当前行的样式复制到目标Excel工作表的第一个非空白行
        //                                        dictRow.Copy(collectedExcelWorksheet.Cells[lastRowIndex + 1, 1]); //将当前行的数据复制到目标Excel工作表的第一个非空白行
        //                                    }

        //                                    FormatExcelWorksheet(collectedExcelWorksheet, headerRowCount, 0); //设置目标Excel工作表格式

        //                                    // 保存目标Excel工作簿文件
        //                                    FileInfo assembledExcelFile = new FileInfo(Path.Combine(targetFolderPath, $"{CleanFileAndFolderName($"{workbookCount++}_{pair.Key}_{excelWorksheet.Name}_{excelWorkbookFileMainName}", 80)}.xlsx")); //获取目标Excel工作簿文件路径全名信息（文件主名为序号加键名加当前工作表名加当前工作簿文件主名后去掉不能作为文件名的字符并截取前80个字符)
        //                                    assembledExcelPackage.SaveAs(assembledExcelFile);
        //                                }
        //                            }
        ...
        //                            using (ExcelPackage assembledExcelPackage = new ExcelPackage()) // 新建Excel包，赋值给目标Excel包变量（为当前工作表创建一个新工作簿）
        //                            {

        //                                foreach (KeyValuePair<string, List<ExcelRangeBase>> pair in dataDict) //遍历所有字典数据
        //                                {
        //                                    // 新建Excel工作表，表名为序号加键名后去掉不能作为工作表名的字符并截取前15个字符，赋值给目标工作表变量
        //                                    ExcelWorksheet collectedExcelWorksheet = assembledExcelPackage.Workbook.Worksheets.Add(CleanWorksheetName($"{worksheetCount++}_{pair.Key}", 15));
        ...
        //                                // 保存目标Excel工作簿文件
        //                                FileInfo assembledExcelFile = new FileInfo(Path.Combine(targetFolderPath, $"{CleanFileAndFolderName($"{workbookCount++}_{excelWorksheet.Name}_{excelWorkbookFileMainName}", 80)}.xlsx")); //获取目标Excel工作簿文件路径全名信息
        //                                assembledExcelPackage.SaveAs(assembledExcelFile);
        ...
        //                            using (ExcelPackage assembledExcelPackage = new ExcelPackage()) // 新建Excel包，赋值给目标Excel包变量（为当前工作表创建一个新工作簿）
        //                            {
        //                                ExcelWorksheet collectedExcelWorksheet = assembledExcelPackage.Workbook.Worksheets.Add("Sheet1");  // 新建Excel工作表
