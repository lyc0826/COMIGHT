...
namespace COMIGHT
{
    public static partial class Methods
    {
        public static async Task FormatWordDocumentsHelperAsync(List<string> filePaths)
        {
            ...
            void Process()
            {
                ...
                try
                {
                    ...
                    // 创建大标题正则表达式变量，匹配模式为：从开头开始，不含2个及以上连续的换行符回车符（允许不连续的换行符回车符）、不含附件标记、非“。”分页符的字符1-150个，换行符回车符，后方出现：换行符回车符
                    ...
                    // 创建0级小标题正则表达式变量，匹配模式为：从开头开始，中英文部分、篇、章、节标记，非“。；;”分页符换行符回车符的字符0-100个，换行符回车符
                    Regex regExHeading0 = new Regex(@"(?<=^|\n|\r)(?:(?:第[ |\t]*[\d一二三四五六七八九十〇零]+[ |\t]*(?:部分|篇|章|节))|(?:part|chapter|section))[^。；;\f\n\r]{0,100}[\n\r]", RegexOptions.Multiline | RegexOptions.IgnoreCase);
                    ...
                    // 创建表格标题正则表达式变量，匹配模式为：从开头开始（总长度1-100个字符），非“。；;”分页符换行符回车符的字符任意多个，“表、单、录、册、回执”，非“。；;”分页符换行符回车符的字符任意多个，换行符回车符
                    Regex regExTableTitle = new Regex(@"(?<=^|\n|\r)(?=.{1,100}[\n\r])[^。；;\f\n\r]*(?:表|单|录|册|回执|table|form|list|rolster|roll)[^。；;\f\n\r]*[\n\r]", RegexOptions.Multiline | RegexOptions.IgnoreCase);
                    ...
                    foreach (string filePath in filePaths) //遍历文件路径列表所有元素
                    {
                        ...
                        // 获取文档大标题正则匹配结果
                        MatchCollection matchesTitles = regExTitle.Matches(documentText);
                        ...
                        foreach (Match matchTitle in matchesTitles) // 遍历所有匹配到的大标题文字
                        {
                            ...
                            if (!selection.Information[WdInformation.wdWithInTable] && pageNum != referencePageNum) //如果当前大标题不在表格内，且与之前已确定的大标题不在同一页（一页最多一个大标题）
                            {
                                ...
                                if (formatTitle) // 如果要设置大标题格式
                                {
                                    ...
                                    MatchCollection matchesCODAddressees = regExCODAddressee.Matches(selection.Text); // 获取选区文字经过中国公文发往单位正则表达式匹配的结果
                                    foreach (Match matchCODAddressee in matchesCODAddressees) // 遍历所有匹配到的中国公文发往单位文字结果
                                    {
                        ...
                        // 获取0级小标题正则匹配结果
                        MatchCollection matchesHeading0s = regExHeading0.Matches(documentText);

                        foreach (Match matchHeading0 in matchesHeading0s)
                        {
                            find.Text = matchHeading0.Value;
                            find.Execute();
                            if (paragraphs[1].Range.Sentences.Count == 1) // 如果小标题所在段落只有一句
                            {
                        ...
                        if (appSettings.DocumentLayoutOption == EnumDocumentLayoutOption.Chinese_Official) // 如果按中国公文版式排版
                        {
                            ...
                            foreach (Match matchCODHeading1_2 in matchesCODHeading1_2s)
                            {
                                ...
                                {
                                    if (paragraphs[1].Range.Sentences.Count == 1) // 如果小标题所在段落只有一句（不是一个段落）
                                    {
                                        paragraphs[1].OutlineLevel = WdOutlineLevel.wdOutlineLevel1; // 将小标题的大纲级别设为1级
                                    }
                                ...
                                else // 否则
                                {
                                    if (paragraphs[1].Range.Sentences.Count == 1) // 如果小标题所在段落只有一句（不是一个段落）
                                    {
                                        paragraphs[1].OutlineLevel = WdOutlineLevel.wdOutlineLevel2; // 将小标题的大纲级别设为2级
                                    }
                        ...
                        else //如果不是中国公文
                        {
                            ...
                            MatchCollection matchesUniveralHeading1_4s = regExUniversalHeading1_4.Matches(documentText); // 获取全文文字经过通用小标题正则表达式匹配的结果

                            foreach (Match matchUniversalHeading1_4 in matchesUniveralHeading1_4s)
                            {
                                find.Text = matchUniversalHeading1_4.Value;
                                find.Execute();

                                // 计算小标题编号中含有几组数字：使用正则表达式以"."分割小标题编号字符串（捕获组1），筛选出不为null或全空白字符的字符串，转换成列表，并统计列表元素个数
                                int universalHeadingNumsCount = Regex.Split(matchUniversalHeading1_4.Groups[1].Value, @"\.")
                        ...
                        foreach (MSWordTable table in msWordDocument.Tables)
                        {
                            ...
                            MatchCollection matchesTableTitles = regExTableTitle.Matches(selection.Text); // 获取选区文字经过表格标题正则表达式匹配的结果
                        ...
                        // 根据是否为中文文档，选择相应的落款正则表达式匹配结果
                        MatchCollection matchesSignOffs = isCnDocument switch
                        {   
                            true => regExCnSignOff.Matches(documentText),
                            false => regExEnSignOff.Matches(documentText)
                        };
            ...
            try
            {
                ...
                for (int j = 1; j <= columnCount; j++) //遍历所有列
                {
                    if (!excelWorksheet.Columns[j].Hidden) //如果当前列不为隐藏列
                    {
                        ...
                        double columnWidth = Math.Clamp(Math.Min(excelWorksheet.Columns[j].Width, averageCharacterCount * 2 + 4), 6, maxColumnWidth);
        ...
        {
            string latestColumnLetter = userRecords.KeyColumnLetter; //读取设置中保存的主键列符
            ...
            userRecords.KeyColumnLetter = columnLetter; // 将对话框返回的列符存入设置
        ...
        {
            string latestOperatingRanges = userRecords.OperatingRanges; //读取用户使用记录中保存的操作区域
            ...
            userRecords.OperatingRanges = operatingRanges; //将对话框返回的操作区域赋值给用户使用记录
        ...
        {
            string latestExcelWorksheetIndexesStr = userRecords.WorksheetIndexesStr; //读取用户使用记录中保存的Excel工作表索引号范围字符串
            ...
            userRecords.WorksheetIndexesStr = excelWorksheetIndexesStr; // 将对话框返回的Excel工作表索引号范围字符串赋值给用户使用记录
            ...
            try
            {
                ...
                foreach (var result in results)
                {
                    ...
                    foreach (var value in values) // 遍历values集合
                    {
                        ...
                        // 将日期字符串按'-'分割成parts (parts[0]为年，parts[1]为月，parts[2]为日)
        ...
        public static DateTime? GetFileInternalDateTime(string filePath)
        {
            try
            {
                ...
                using var shellFile = ShellFile.FromFilePath(filePath); // 创建ShellFile对象
                {
                    foreach (var timeProperty in GetPropertyPriorityByFileType(ext)) // 遍历时间属性优先级列表
                    {
                        var dateTime = TryReadShellDate(shellFile, timeProperty); // 尝试读取时间属性的时间
                        if (dateTime.HasValue) // 如果时间属性有值，则返回该时间
                        {
        ...
        {
            string lastestHeaderFooterRowCountStr = userRecords.HeaderAndFooterRowCountStr; //读取设置中保存的表头表尾行数字符串
            ...
            userRecords.HeaderAndFooterRowCountStr = headerFooterRowCountStr; // 将对话框返回的表头、表尾行数字符串存入设置
        ...
        public static string RemoveMarkdownMarks(this string inText)
        {
            ...
            // 移除行内代码和代码块引用符号 - 代码引用符号正则表达式匹配模式为：任意字符任意多个（尽量少匹配）（捕获组1），“`”1-3个（捕获组2），任意字符任意多个（尽量少匹配）（捕获组3），捕获组2，任意字符任意多个（尽量少匹配）（捕获组4）；将匹配到的字符串替换为捕获组1、3、4合并后的字符串
        ...
        public static List<string>? SelectFiles(EnumFileType fileType, bool isMultiselect, string dialogTitle)
        {
            ...
            string initialDirectory = userRecords.FolderPath; //获取保存在设置中的文件夹路径
            ...
            {
                userRecords.FolderPath = Path.GetDirectoryName(openFileDialog.FileNames[0])!; // 将本次选择的文件的文件夹路径保存到设置中
        ...
        {
            string initialDirectory = userRecords.FolderPath; // 读取用户使用记录中保存的文件夹路径
            ...
            if (openFolderDialog.ShowDialog() == true) // 如果对话框返回值为true（点击OK）
            {
                ...
                userRecords.FolderPath = folderPath;  // 将文件夹路径赋值给用户使用记录
