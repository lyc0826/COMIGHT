...
namespace COMIGHT
{
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e) 
        {
            ...
            base.OnStartup(e); // 如果是第一个实例，调用基类的 OnStartup 方法继续正常的应用程序启动流程
        }
        ...
        private void ActivateExistingWindow()
        {
            ...
            foreach (var process in processes)
            {
                ...
                {
                    // 调用 Windows API 恢复窗口显示状态（如果窗口被最小化），参数1: 窗口句柄；参数2: 显示命令（SW_RESTORE = 9，表示恢复窗口）
                    ShowWindow(process.MainWindowHandle, SW_RESTORE);
                    
                    SetForegroundWindow(process.MainWindowHandle); // 调用 Windows API 将指定窗口设置为前台窗口（激活窗口）
